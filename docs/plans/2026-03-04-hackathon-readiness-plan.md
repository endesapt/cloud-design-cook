...
6. Добавлен client-side fallback для устаревшей сессии после пересоздания БД: при `UNAUTHORIZED` и при `NOT_FOUND` на `/api/v1/quota` пользователь перенаправляется на `/login`.
